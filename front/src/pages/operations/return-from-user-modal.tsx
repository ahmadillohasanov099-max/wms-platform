...
export default function ReturnFromUserModal({ open, onClose }: Props) {
  const { t } = useTranslation();
  const queryClient = useQueryClient();
  const [selectedUser, setSelectedUser] = useState('');
  const schema = z.object({
    userId: z.string().min(1, t('operations.validationEmployee')),
  ...
  const { data: assignmentsData } = useQuery({
    ...
    queryFn: () => usersApi.getAssignments(selectedUser),
  ...
  const { mutate, isPending } = useMutation({
    mutationFn: (data: FormData) => operationsApi.returnFromUser(data),
    onSuccess: () => {
  ...
  return (
    <Modal
      open={open}
      onClose={() => { reset(); setSelectedUser(''); onClose(); }}
      title={t('operations.returnFromUser')}
      subtitle={t('operations.returnFromUserSubtitle')}
      size="md"
      footer={
        <>
          <Button variant="outline" onClick={() => { reset(); setSelectedUser(''); onClose(); }} disabled={isPending}>
            {t('common.cancel')}
          </Button>
    ...
    >
      <div className="space-y-4">
        ...
        <Select
          ...
          placeholder={selectedUser ? t('operations.assetPlaceholder') : t('operations.assetSelectUserFirst')}
