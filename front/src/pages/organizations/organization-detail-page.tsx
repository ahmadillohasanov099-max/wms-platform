...
export default function OrganizationDetailPage() {
  ...
  if (!org) {
    return (
      <div className="p-8 text-center">
        <p className="text-gray-500">{t('organizations.notFound')}</p>
  ...
  const columns: Column<Inventory>[] = [
    {
      key: 'product',
      title: t('organizations.colProduct'),
      render: (_, row) => (
        <div>
          ...
          <div className="flex items-center gap-2 mt-1">
            ...
            <span className="text-2xs text-gray-400">
              {t('organizations.unitPrefix', { unit: row.product?.unit || t('organizations.defaultUnit') })}
            </span>
          </div>
        </div>
      ),
    ...
    {
      key: 'quantity',
      title: t('organizations.colAvailableQty'),
      render: (val, row) => (
        <div className="flex items-center gap-2">
          ...
          >
            {val} {row.product?.unit || t('organizations.defaultUnit')}
          </span>
          {row.isLowStock && (
    ...
    {
      key: 'minLevel',
      title: t('organizations.colMinLevel'),
      render: (val, row) => (
        <span className="text-xs text-gray-600 dark:text-gray-400 font-medium">
          {val || 0} {row.product?.unit || t('organizations.defaultUnit')}
        </span>
      ),
    },
    {
      key: 'unitPrice',
      title: t('organizations.colUnitPrice'),
      render: (val) => (
    ...
    {
      key: 'totalValue',
      title: t('organizations.colTotalValue'),
      render: (val) => (
    ...
    {
      key: 'actions',
      title: t('organizations.colAssets'),
      ...
      <Card className="p-6 bg-gradient-to-r from-teal-600/10 via-teal-500/5 to-transparent border border-gray-200/90 dark:border-white/15">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              ...
              <div>
                ...
                {org.code && (
                  <span className="inline-block mt-0.5 text-xs font-mono px-2 py-0.5 rounded bg-white/80 dark:bg-slate-900/80 border border-gray-200 dark:border-slate-800 text-gray-700 dark:text-gray-300 font-bold">
                    {t('organizations.codeLabel', { code: org.code })}
                  </span>
                )}
              </div>
            </div>
          ...
          <Badge variant={org.type === 'MINISTRY' ? 'info' : 'gray'} className="self-start md:self-auto text-xs px-3 py-1">
            {org.type === 'MINISTRY' ? t('organizations.typeMinistryBadge') : t('organizations.typeSubOrgBadge')}
          </Badge>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 mt-6 pt-6 border-t border-gray-200/60 dark:border-gray-800">
          <div className="p-3.5 bg-white/90 dark:bg-slate-900/90 rounded-xl border border-gray-200/80 dark:border-slate-800">
            <div className="flex items-center gap-1.5 text-gray-500 text-xs font-medium">
              ...
              {t('organizations.totalProducts')}
            </div>
            <p className="text-xl font-bold text-gray-900 dark:text-gray-100 mt-1">
              {t('organizations.countItems', { count: summary.totalItems })}
            </p>
          </div>
          ...
          <div className="p-3.5 bg-white/90 dark:bg-slate-900/90 rounded-xl border border-gray-200/80 dark:border-slate-800">
            <div className="flex items-center gap-1.5 text-gray-500 text-xs font-medium">
              ...
              {t('organizations.fixedAssets')}
            </div>
            <p className="text-xl font-bold text-gray-900 dark:text-gray-100 mt-1">
              {t('organizations.countTypes', { count: summary.beriladiganCount })}
            </p>
          </div>
          ...
          <div className="p-3.5 bg-white/90 dark:bg-slate-900/90 rounded-xl border border-gray-200/80 dark:border-slate-800">
            <div className="flex items-center gap-1.5 text-gray-500 text-xs font-medium">
              ...
              {t('organizations.consumables')}
            </div>
            <p className="text-xl font-bold text-gray-900 dark:text-gray-100 mt-1">
              {t('organizations.countTypes', { count: summary.sarflanadiganCount })}
            </p>
          </div>
          ...
          <div className="p-3.5 bg-white/90 dark:bg-slate-900/90 rounded-xl border border-gray-200/80 dark:border-slate-800">
            <div className="flex items-center gap-1.5 text-gray-500 text-xs font-medium">
              ...
              {t('organizations.lowStock')}
            </div>
            <p className="text-xl font-bold text-amber-600 dark:text-amber-400 mt-1">
              {t('organizations.countItems', { count: summary.lowStockCount })}
            </p>
          </div>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-3">
          <div className="w-full sm:w-80">
            <input
              ...
              placeholder={t('organizations.searchProductPlaceholder')}
          ...
          <div className="flex flex-wrap items-center gap-2 w-full sm:w-auto">
            ...
            >
              ...
              <option value="BERILADIGAN">{t('organizations.typeFixedAssets')}</option>
              <option value="SARFLANADIGAN">{t('organizations.typeConsumables')}</option>
            </select>
            ...
            >
              ...
              {t('organizations.filterLowStock', { count: summary.lowStockCount })}
            </button>
          </div>
        </div>
      </Card>
      ...
      <Card className="overflow-hidden">
        <div className="p-4 border-b border-gray-100 dark:border-gray-800 flex items-center justify-between">
          <h3 className="font-bold text-gray-900 dark:text-gray-100 text-sm flex items-center gap-2">
            ...
            <span>{t('organizations.warehouseTableTitle', { count: filteredInventory.length })}</span>
          </h3>
        </div>
